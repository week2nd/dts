			...
			<table class="table table-borderless table-data3" style="text-align: center">
				...
				<c:forEach items="${matchListAd}" var="match">
					<tr id="m${match.gameId}">
						...
						<td class="result"><button type="button" class="btn btn-primary setResultBtn" disabled="disabled" >구매결과입력</button></td>
						</c:if>
						<c:if test="${match.checkResult!=0}">
						<td class="result"><button type="button" class="btn btn-primary setResultBtn" >구매결과입력</button></td>
						</c:if>					
						<c:if test="${match.checkReturn==0}">
						<td class="money"><button type="button" class="btn btn-primary setMoneyBtn">적중금액지급</button></td>
						</c:if>
						<c:if test="${match.checkReturn!=0}">
						<td class="money"><button type="button" class="btn btn-primary setMoneyBtn" disabled="disabled">적중금액지급</button></td>
						</c:if>
					</tr>
				</c:forEach>
